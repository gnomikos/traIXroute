...
class traIXroute():
        ...
        for index, entry in enumerate(self.input_list[indexes[0]:indexes[1]]):
            ...
            if self.import_flag == 1:
                ...
                if self.traixparser.flags['outputfile_txt'] or not self.traixparser.flags['silent']:
                    output.print_traIXroute_dest(
                        self.traixparser, db_extract, self.dns_print, dst_ip, src_ip, info)
                ...
                if self.traixparser.flags['outputfile_txt'] or not self.traixparser.flags['silent']:
                    output.print_traIXroute_dest(
                        self.traixparser, db_extract, self.dns_print, dst_ip, src_ip)
            else:
                src_ip = ''
                ...
                if self.traixparser.flags['outputfile_txt'] or not self.traixparser.flags['silent']:
                    output.print_traIXroute_dest(
                        self.traixparser, db_extract, self.dns_print, dst_ip)
            ...
            if len(ip_path):
                ...
                if self.traixparser.flags['outputfile_txt'] or not self.traixparser.flags['silent']:
                    output.print_path_info(
                        ip_path, delays_path, path_info_extract, self.traixparser)

                rule_hits = self.detection_rules.resolve_path(
                    ip_path, output, path_info_extract, db_extract, self.traixparser)

                if self.import_flag == 2 or self.ripe == 1:
                    output.buildJsonRipe(
                        entry, path_info_extract.asn_list, db_extract)
                else:
                    output.buildJson(
    ...
    def main(self):
        ...
        # Initiates the tool with the output directories and all the proper
        # configurations.
        ...
        if useTraIXroute:
            ...
            if self.import_flag:
                ...
                if import_is_dir:
                    self.dir_walk(
                        ...
                        self.arguments,
                        manager,
                        self.traixroute_core)
                # Analyze the given file.
                else:
                    [input_list, flag] = self.json_handle.import_IXP_dict(
                        self.arguments)
                    if flag:
                        print(
                            'WARNING:',
                            self.arguments +
                            ' file not found or has invalid json format. Exiting.')
                        sys.exit(0)
                    self.traixroute_core(
                        homepath,
                        input_list,
            ...
            # Case: when a ripe atlas measurement is fetched to be analyzed.
                ...
                self.traixroute_core(
                    ...
                    self.arguments,
                    manager)
            # Case: when a ripe atlas measurement is created and then it is
            # fetched to be analyzed.
            elif self.ripe == 2:
                self.ripe_handle = handle_ripe.handle_ripe(self.config)
                input_list = self.ripe_handle.create_measurement(
                    self.arguments)
                self.traixroute_core(
                    homepath,
                    input_list,
                ...
                # Check IP or FQDN format consistency for the given
                # destinations.
        ...
        # Empty database. In case of using threads only one database instance
        # is used.
        if self.mode == 'thread':
            self.db_extract.clean()

    # Finds all the files in directories and subdirectories when a directory
    ...
    def traixroute_core(self, homepath, input_list,
                        useTraIXroute, arguments, manager):
        ...
        if self.enable_stats:
            final_rules_hit = [0] * len(self.detection_rules.rules)
        ...
        size_of_sublist = math.ceil(max(size_of_biglist,
                                        self.config["num_of_cores"]) / min(size_of_biglist,
                                                                           self.config["num_of_cores"]))
        sublisted_data = [[x, x + size_of_sublist]
                          for x in range(0, size_of_biglist, size_of_sublist)]
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=self.config["num_of_cores"]) \
                ...
                as executor:
            for [rule_hits, json_obj, txt_obj] in executor.map(
                    self.analyze_measurement, sublisted_data):
                if self.traixparser.flags['outputfile_json']:
                    json_data.append(json_obj)
                if self.traixparser.flags['outputfile_txt']:
                    txt_data.append(txt_obj)
        ...
        output.export_results_to_files(
            json_data,
            txt_data,
            self.traixparser,
            homepath,
