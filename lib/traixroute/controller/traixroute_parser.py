...
class traixroute_parser():
    ...
    def parse_input(self):
        ...
        parser.add_argument(
            ...
            '--version',
            action='version',
            version='current version of traixroute: ' +
            self.version)
        parser_mode = parser.add_mutually_exclusive_group(required=True)
        parser_mode.add_argument(
            '-thread',
            action='store_true',
            help='Enables threads for parallel analysis. This mode is more conservative in terms of performance and is recommended in case of memory limitations.')
        parser_mode.add_argument(
            '-process',
            action='store_true',
            help='Enables processes for parallel analysis. This mode is quite memory hungry to maximize the analysis performance.')
        ...
        # Parameterize arguments from subparser import
            ...
            if (options.parse_json is not None):
                ...
                self.flags['useTraiXroute'] = True
                self.flags['showSourceIP'] = True
                self.flags['import_is_dir'] = True if os.path.isdir(
                    self.arguments) else False
