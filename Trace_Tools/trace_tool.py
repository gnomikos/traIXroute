...
class trace_tool():
    ...
    def trace_call(self,IP_name,classic,arguments):
        ...
        #A domain name has been given as destination to send the probe instead of an IP address.
        ...
        if  not string_handle.is_valid_ip_address(IP_name,'IP'):
            try:
                ...
                print('--> '+IP_name+' has wrong address format. Expected an IPv4 format or a valid url.')
                return [],[]
        ...
        if classic:      
            ...
            if not len(route):  print('--> Scamper returned an empty IP path. You may use "sudo" in the beginning or the scamper arguments might be wrong.')
        elif not classic:
            [route,mytime]=self.traceroute_call(IP_name,arguments)
            if not len(route):  print('--> Traceroute returned an empty IP path. You may use "sudo" in the beginning.')
    ...
    def scamper_call(self,IP_name,arguments):
        ...
        if arguments=='':
            try:
                whole=subprocess.check_output(str('scamper -i '+IP_name).split(" "),shell=False)
            except:
                try:
                    print('Scamper failed. Trying to run with sudo..')
                    whole=subprocess.check_output(str('sudo scamper -i '+IP_name).split(" "),shell=False)
            ...
            try:
                whole=subprocess.check_output(['scamper', '-I', arguments],shell=False)
            except:
                try:
                    print('Scamper failed. Trying to run with sudo..')
                    whole=subprocess.check_output(['sudo','scamper', '-I', arguments],shell=False)
                except Exception as e:
                    print('Error occured. Exiting.',e)
                    sys.exit(0)
    ...
    def traceroute_call(self,IP_name,arguments):
        ...
        if arguments!='':
            try:
                whole=subprocess.check_output(str('traceroute ' +IP_name+' '+arguments).split(" "),shell=False)
            except:
                try:
                    print('Traceroute failed. Trying to run with sudo..')
                    whole=subprocess.check_output(str('sudo traceroute ' +IP_name+' '+arguments).split(" "),shell=False)
                except Exception as e:
                    print('Error occured. Exiting.',e)
                    sys.exit(0)
        else:   
            try:
                whole=subprocess.check_output(str('traceroute ' +IP_name).split(" "),shell=False)
            except:
                try:
                    print('Traceroute failed. Trying to run with sudo..')
                    whole=subprocess.check_output(str('sudo traceroute ' +IP_name).split(" "),shell=False)
                except Exception as e:
                    print('Error occured. Exiting.',e)
                    sys.exit(0)
