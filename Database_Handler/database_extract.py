...
from Database_Handler import handle_json,handle_pch,handle_pdb,handle_ripe,dict_merger,handle_complementary,handle_remote
...
class database():
    ...
    def __init__(self, traIXroute_parser, mypath, config, outcome):
        ...
        # self.remote_peering: A dictionary {IP}={IXP short name: {IXP Country, IXP City}} containing remote peering related information.
       ...
       #     self.mypath = self.mypath + '/Database/Default/'
        ...
        if (lst_modified and not chk_update and not self.merge_flag) or not self.outcome:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.config["num_of_cores"]) as executor:
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'Merged/IXPIP2ASN.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'Merged/trIX_subnet2name.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'Merged/asn_memb.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'Merged/sub2country.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'Merged/routeviews.json'))
        ...
        if (not lst_modified or flag or chk_update or self.merge_flag ) and self.outcome:
            ...
            if not exists(self.mypath+'Merged'):
                makedirs(self.mypath+'Merged')
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.config["num_of_cores"]) as executor:
                executor.submit(json_handle.export_IXP_dict,self.asnmemb,self.mypath+'Merged/asn_memb.json')
                executor.submit(json_handle.export_IXP_dict,routeviews_dict,self.mypath+'Merged/routeviews.json')
                executor.submit(json_handle.export_IXP_dict,self.final_ixp2asn,self.mypath+'Merged/IXPIP2ASN.json')
                executor.submit(json_handle.export_IXP_dict,self.final_sub2name,self.mypath+'Merged/trIX_subnet2name.json') 
                executor.submit(json_handle.export_IXP_dict,final_subnet2country,self.mypath+'Merged/sub2country.json')
        ...
        self.remote_peering = handle_remote.handle_remote(self.traixpath).handle_import(json_handle)
