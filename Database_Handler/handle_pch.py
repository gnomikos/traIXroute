...
class pch_handle():
    ...
    def pch_handle_main(self,ixp_membership,ixp_subnet,ixp_excha,mypath,reserved_tree,add_info_tree,country2cc,db_path):
        '''
        ...
        [long_mem,IXP_region]=self.pch_handle_long(ixp_excha,mypath,country2cc)
        [pch_final,pch_region,Subnet_names]=self.pch_handle_sub(ixp_subnet,mypath,reserved_tree,long_mem,IXP_region)
        ixp_mem=self.pch_handle_ixpm(ixp_membership,mypath,reserved_tree,add_info_tree)
        
        return (pch_final,ixp_mem,pch_region)


    def pch_handle_ixpm(self,filename,mypath,reserved_tree,add_info_tree):
        '''
                        ...
                        for snode in subnet:
                            ...
                            if hstring.is_valid_ip_address(snode,'Subnet'):
                                ...
                                if (inode in tree and inode not in ixpip2asn.keys() and inode not in dumped_ixps and inode not in reserved_tree) or (inode in add_info_tree):
                                    ixpip2asn[inode]=[temp_string[3].replace(' ','')]
    ...
    def pch_handle_sub(self,filename,mypath,reserved_tree,long_mem,IXP_region):
        '''
                    ...
                    if ips!='' and handled_string.string_comparison(temp_string[2],'Active'):
                        if handled_string.is_valid_ip_address(ips,'Subnet') and ips not in subnets.keys():
                            if mykey in long_mem.keys():
                                ...
                                [long_name,short_name]=handled_string.clean_long_short(long_mem[mykey],temp_string[1])
                            ...
                            for IXP in subnets[ips]:
                                assigned_tuple=assigned_tuple+handled_string.assign_names(IXP[0],short_name,IXP[1],long_name)
    ...
    def pch_handle_long(self,filename,mypath,country2cc):
        '''
        ...
        for line in doc:
            ...
            if len(temp_string)>6:
                if handled_string.string_comparison(temp_string[5],'Active'):
                    ...
                    country= re.sub('([^\s\w]|_)+',' ', temp_string[2].strip())
    ...
    def file_opener(self,filename,mypath,option):
        ...
        try:
            doc=open(mypath+filename+'.csv')
        except:
            download=download_files.download_files(self.config)
            print (filename+' was not found. Downloading...')
            if not download.download_pch(mypath,option):
                print(filename+" was not downloaded. Copying from the default database.")
                try:
                    copyfile(self.db_path+'Default/'+filename+'.csv',self.db_path+'User/'+filename+'.csv')
                except:
                    print ('Could not copy '+filename+'from the default database.')
            try:
                doc=open(mypath+filename+'.csv')
            except:
