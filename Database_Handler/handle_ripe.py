...
class handle_ripe():
    ...
    def create_measurement(self, kwargs):
        ...
        if(is_success): print('Please wait for ripe to complete the measurements and run traIXroute again with: ripe -r \'{\"msm_id\":'+str(response['measurements'][0])+'}\'')
        else: print('Please revise the arguments you have given. Ripe does not accept these arguments or it may be unavailable at this time.')
        sys.exit(0)
    ...
    def choose_ip(self,packet):
        '''
        Chooses which ip to return for a given traceroute hop.
        Input: 
        ...
        iterator=0
        ip=None
        while (iterator < 3 and ip is None):
            try:
                sel_packet = packet[iterator]
                ip=sel_packet.origin
                delay=str(sel_packet.rtt)
            except:
                pass
            iterator+=1   
        if (ip is None):
