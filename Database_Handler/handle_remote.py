...
class handle_remote():
    ...
    def __init__(self,path):
        ...
        self.directory = path+'Database/RemotePeering/'
        # The filename containing the total infomation concerning the remote peering interfaces.
    ...
    def extract_rp_per_ixp(self,files,json_handle):
        ...
        for json_file in files:
            data = json_handle.import_IXP_dict(self.directory+json_file)[0] 
            for entry in data[3]:
                rp_dataset.setdefault(entry["ip"],{}).update({str((data[0],data[1],data[2])):entry})
    ...
    def handle_import(self,json_handle):
        ...
        if self.rp_dataset_json in files:            
            return json_handle.import_IXP_dict(self.directory+self.rp_dataset_json)[0]
        else:
            rp_database = self.extract_rp_per_ixp(files,json_handle)
            json_handle.export_IXP_dict(rp_database,self.directory+self.rp_dataset_json)
            return rp_database
