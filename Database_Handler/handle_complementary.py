...
class asn_handle():
    ...
    def routeviews_extract(self,filename,mypath,reserved_sub_tree,config,db_path):
        '''
...
class reserved_handle():
    ...
    def __init__(self):
        ...
        self.reserved_list=['0.0.0.0/8','10.0.0.0/8','100.64.0.0/10','127.0.0.0/8','169.254.0.0/16','172.16.0.0/12','192.0.0.0/24','192.0.2.0/24','192.88.99.0/24','192.168.0.0/16','198.18.0.0/15','198.51.100.0/24','203.0.113.0/24','224.0.0.0/4','240.0.0.0/4','255.255.255.255/32']
            ...
            #If a subprefix exist.
                ...
                for IXP1 in Stree[subnet]:
                    for IXP2 in Sub[subnet]:
                        assign_tuple=assign_tuple+handle_string.assign_names(IXP1[1],IXP2[1],IXP1[0],IXP2[0])
                ...
                for i in range(0,len(assign_tuple)-1):
                    for j in range(i+1,len(assign_tuple)):                    
                        if len(handle_string.assign_names(assign_tuple[i][0],assign_tuple[j][0],assign_tuple[i][1],assign_tuple[j][1]))==1:
                            if j not in deleted:
                                deleted=[j]+deleted       
                for node in deleted: del assign_tuple[node]
...
class extract_additional_info():
    ...
    def __init__(self):
        ...
        self.Subnet={}
        # additional_info_tree: A Subnet Tree containing all the IXP subnets provided by the user.
        self.additional_info_tree=SubnetTree.SubnetTree()
         # pfx2cc: A Dictionary containing all the IXP subnets or IPs to cities provided by the user.
        self.pfx2cc = {}
        # additional_info_help_tree: A subnetTree containing [IXP Subnet]=IXP Subnet
