...
class peering_handle():
    ...
    def peering_handle(self,filename_peer_name,filename_peer_ix,filename_peer_pfx,filename_peer_ixlan,add_subnet_tree,mypath,reserved_tree,config,country2cc,db_path):
        '''
        ...
        json_names=self.extract_json_data(filename_peer_name,mypath,2,config)
        [id_to_names,id_to_region]=self.extract_names(json_names,country2cc)

        json_ixlan=self.extract_json_data(filename_peer_ixlan,mypath,4,config)
        ixlan_dict=self.extract_ixlan(json_ixlan)

        json_pfx=self.extract_json_data(filename_peer_pfx,mypath,1,config)
        (sub2names,temp_subnet_tree,subnet2region)=self.extract_pfx(json_pfx,ixlan_dict,id_to_names,reserved_tree,id_to_region)

        json_ip=self.extract_json_data(filename_peer_ix,mypath,3,config)
        (ip2asn)=self.extract_ip(json_ip,temp_subnet_tree,add_subnet_tree,reserved_tree)

        return(sub2names,ip2asn,subnet2region)

    
    def extract_json_data(self,filename_peer_name,mypath,option,config):
        '''
        ...
        try:
            with open (mypath+filename_peer_name) as data_file:
                obj = ujson.load(data_file)
        except:
            download=download_files.download_files(config)
            print(filename_peer_name+' was not found.')

            if not download.download_peering(mypath,option):
                print("Could not download "+filename_peer_name+". Copying from the default database.")
                try:
                    copyfile(self.db_path+'Default/'+filename_peer_name,self.db_path+'User/'+filename_peer_name)
                except:
                    print ('Could not copy '+filename_peer_name+' from the default database.')

            try:
                with open (mypath+filename_peer_name) as data_file:
                    obj = ujson.load(data_file)
            except:
                print('Could not open '+filename_peer_name+'. Exiting.')
                exit(0)
        return (obj['data'])
    ...
    def extract_names(self,json_names,country2cc):
        ...
        for node in json_names:
            [long_name,short_name]=handle_string.clean_long_short(node['name_long'],node['name'])
                ...
                if handler.is_valid_ip_address(s,'Subnet'):
                    ...
                    if ixlan_id in ixlan_dict.keys():
                        ...
                        if ix_id in id_to_names.keys() and s not in pfxs_dict:
                            ...
                            assign_tuple=[]
                            for IXP in pfx_dict:
                                assign_tuple=assign_tuple+handler.assign_names(IXP[1],id_to_names[ix_id][1],IXP[0],id_to_names[ix_id][0])
