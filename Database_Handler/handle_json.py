...
import ujson, os, SubnetTree, sys
...
class handle_json():
    ...
    def export_trace_from_file(self,trace):
        ...
        try:
            ...
            trace_dst=trace['dst']
            trace_src=trace['src']
            if( 'info' in trace.keys() ): trace_info=trace['info']
        ...
        if not tmp_list: return current_trace,current_info,trace_dst,trace_src,trace_info
        ...
        for i in range(0,tmp_list[-1][0] + 1):
            ...
            try:
                ...
                if len(hop)>0:
                    ...
                    print('Expected at least one hop in '+ str(i+1) +' in '+trace+'. Hop is ignored.')    
                    IP='*'
                    info=' '
            except:
                IP='*'
            ...
            if flag:
                try:
                    ...
                    if( 'info' in hop_to_parse.keys() ): info=str(hop_to_parse['info'])
                except:
                    print('Wrong format, hop: '+ str(i+1)+'. Exiting.')
                    print(trace)
                    os._exit(0)
    ...
    def export_trace_from_ripe_file(self,trace):
        '''
        Exports the traces from an input ripe-json-based file to a new file, in json format too. As input example, see Examples/test_traceroute_paths.json
        Input:
            a) trace: The input traceroute paths.
        Output:
            a) current_trace: A list [IP1,IP2,...IPN].
        ...
        try:
            ...
            if( 'msm_name' in trace.keys() ): trace_info=trace['msm_name']
            path_to_parse=trace['result']
            len_path=len(path_to_parse)
        except:
            print('Wrong json format. Exiting.')
