...
from Controller import traIXroute_output,string_handler
from Detection_Handler.remote_peering import remote_peering
...
class detection_rules():
    ...
    def rules_extract(self,file):
        ...
        try:
            ...
            sys.exit(0)
        [delimeters1,expressions]=self.load_syntax_rules ('Detection_Handler/expressions.txt','Detection_Handler/delimeters.txt')
        rules=f.read()
        rules=rules.split('\n')
        ...
        for i in range(0,len(rules)):
            ...
            if temp[0]!='':
                ...
                temp=(rules[i].split(':'))
                if len(temp)!=2:
                    print('-->Rule in line '+(str(i+1))+'not included. Expected one condition and one assessment part respectively.')
                    flag=False      
                array=temp[0].split('-')
    ...
    def resolve_path(self,path,mypath,output,path_info_extract, db_extract, traIXparser):
        '''
...
#                if 'IXP_IP' in rule[i] and '!AS_M' in rule[i] and 'IXP prefix' not in encounter_type[path_cur+i-1]:
    ...
    def load_syntax_rules (self,filename1,filename2):
        ...
        try: 
            file_ex=open(sys.path[0]+'/'+filename1)
        except:
            print(filename1+' was not found. Exiting.')
            sys.exit(0)
        try: 
            file_del=open(sys.path[0]+'/'+filename2)
        except:
